...
export class ExpandedResultContentComponent implements OnInit, OnDestroy {
    ...
    get mayAnyItemBeBugged() {
        return (
            (this.element?.items.flat().filter((d: ResultItem) => d.mayBeBugged).length || 0) > 0
        );
    }
    ...
    async moveItems(equip = false) {
        ...
        for (let item of items) {
            ...
            let success = await this.bungieApi.transferItem(
                item.itemInstanceId,
                characterId,
                equip
            );
        ...
        if (fragments.length) {
            ...
            const subclassHashes: {
                [characterClass: number]:
                    | { [modifierType: number]: number | undefined }
                    | undefined;
            } = {
            ...
            (
                this.element?.items.filter(
                    (i) =>
                        (!i[0].masterworked &&
                            !i[0].exotic &&
                            this.config_assumeLegendariesMasterworked) ||
