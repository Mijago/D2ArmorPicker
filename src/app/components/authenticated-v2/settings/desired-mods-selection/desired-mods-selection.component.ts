...
export class DesiredModsSelectionComponent implements OnInit, OnDestroy {
    ...
    constructor(private config: ConfigurationService) {
        ...
        let combatStyleMods = modifiers.filter(
            (value) => value.type == ModifierType.CombatStyleMod
        );
    ...
    setElement(element: ModifierType) {
        ...
        this.config.modifyConfiguration((c) => {
            const pos = c.enabledMods.filter(
                (m) =>
                    ModInformation[m].type != ModifierType.CombatStyleMod &&
                    ModInformation[m].type != element
            );
