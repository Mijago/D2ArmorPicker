    ...
    private filterItems(armorItems: LocalArmorInfo[]) {
        if (!!this.armorName)
            armorItems = armorItems.filter(
                (i) => i.name.toLowerCase().indexOf(this.armorName!) > -1
            );
        if (!!this.armorHash)
            armorItems = armorItems.filter(
                (i) => (i.hash || 0).toString().indexOf(this.armorHash!) > -1
            );
        if (!!this.armorId)
            armorItems = armorItems.filter(
        ...
        if ((this.anyPlugWithN ?? 0) > 0)
            armorItems = armorItems.filter(
                (i) =>
                    (i.statPlugHashes || []).filter(
                        (pl) => this.getPlugSum(pl) >= (this.anyPlugWithN || 0)
                    ).length > 0
            );
        ...
        if ((this.anyPlugBelowN ?? 0) < 17)
            armorItems = armorItems.filter(
                (i) =>
                    (i.statPlugHashes || []).filter(
                        (pl) => this.getPlugSum(pl) <= (this.anyPlugBelowN || 0)
                    ).length > 0
            );
        ...
        if ((this.allPlugsWithN ?? 0) > 0)
            armorItems = armorItems.filter(
                (i) =>
                    (i.statPlugHashes || []).filter(
                        (pl) => this.getPlugSum(pl) < (this.allPlugsWithN || 0)
                    ).length == 0
            );
        if ((this.allPlugsBelowN ?? 0) < 17)
            armorItems = armorItems.filter(
                (i) =>
                    (i.statPlugHashes || []).filter(
                        (pl) => this.getPlugSum(pl) > (this.allPlugsBelowN || 0)
                    ).length == 0
            );
