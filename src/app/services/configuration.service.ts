...
export class ConfigurationService {
    ...
    checkAndFixOldSavedConfigurations(c: StoredConfiguration) {
        c.configuration = Object.assign(
            BuildConfiguration.buildEmptyConfiguration(),
            c.configuration
        );
        if (c.configuration.hasOwnProperty("minimumStatTier")) {
            let tiers = (c.configuration as any).minimumStatTier as EnumDictionary<
                ArmorStat,
                number
            >;
            c.configuration.minimumStatTiers[ArmorStat.Mobility].value = tiers[ArmorStat.Mobility];
            c.configuration.minimumStatTiers[ArmorStat.Resilience].value =
                tiers[ArmorStat.Resilience];
            c.configuration.minimumStatTiers[ArmorStat.Recovery].value = tiers[ArmorStat.Recovery];
            c.configuration.minimumStatTiers[ArmorStat.Discipline].value =
                tiers[ArmorStat.Discipline];
            c.configuration.minimumStatTiers[ArmorStat.Intellect].value =
                tiers[ArmorStat.Intellect];
        ...
        c.configuration.enabledMods = c.configuration.enabledMods.filter(
            (v) => !!ModInformation[v]
        );
